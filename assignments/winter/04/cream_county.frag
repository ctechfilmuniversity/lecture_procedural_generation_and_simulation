...
void main() {
    ...
    vec2 mount2_pos = xy;
    vec2 mount3_pos = xy;
    float mountain0 = perlin_mountains(vec2(mount0_pos.x+=u_time*0.02, mount0_pos.y), 2.0, .45);
    float mountain1 = perlin_mountains(vec2(mount1_pos.x+=u_time*0.05, mount1_pos.y)-10.0, 2.0, .4);
    float mountain2 = perlin_mountains(vec2(mount2_pos.x+=u_time*0.025, mount2_pos.y)-20.0, 2.0, 0.475);
    float mountain3 = perlin_mountains(vec2(mount3_pos.x+=u_time*0.0125, mount3_pos.y)-30.0, 2.0, .5);
    
    //Sets up the waves.
    xy.x+=u_time*0.025;
    float wave0 = perlin_waves(xy, 5.0, 4.0, u_time*7.0)*0.005;
    float wave1 = perlin_waves(xy, 10.0, 3.0, (u_time*-1.0)*7.0)*0.0025;
    float wave2 = perlin_waves(xy, 15.0, 3.0, u_time*5.0)*0.00125;
