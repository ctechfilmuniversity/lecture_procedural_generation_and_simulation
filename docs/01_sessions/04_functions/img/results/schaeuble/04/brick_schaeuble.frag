...
// gain and bias used for smoothing ken perlins smoothing functions
...
void main()
{
    ...
    // create divisions along the x axis by dividing the window width by the desired brick width
    // same thing for the height of the brick / window height
    float x = coord.x / brick_mortar_w;
    float y = coord.y / brick_mortar_h;
    // float x = coord.x;
    // float y = coord.y;

    // create a continuous step in values along the y axis
    float y_index = floor(y);
    // for every even y_index step, offset the x coord by 0.5
    if( mod(y_index, 2.0 ) == 0.0)
    {
        x += 0.5;
    }

    // Replicate the coloring for each step along the x axis according to floor(x)
    x -= floor(x);
    // Replicate the coloring for each step along the x axis according y_index
    y -= y_index;
    ...
    // Define the color gradient smoothing, which creates the pattern of brickwork
    // Put in rough english, how much of the x,y cell division is white,
    // and where and when does the transition to black occure
